...
.NOTES
    ...
    Version: 3.2 - Added Original Source Line Column
#>

# ============================================
# CONFIGURATION
# ============================================
$InputFile = "rhsa_list2.txt"
$OutputFile = "rhsa_vulnerability_report_$(Get-Date -Format 'ddMMyyyy_HHmmss').csv"
...
function Extract-RHSAIDsWithSource {
    param (
        [Parameter(Mandatory=$true)]
        [string]$FilePath
    )
    ...
    try {
        ...
        foreach ($line in $content) {
            ...
            # Find all RHSA IDs in this line
            ...
            foreach ($match in $regexMatches) {
                ...
                # Store RHSA ID with its original source line
        ...
        # Remove duplicates based on RHSA_ID (keep first occurrence)
        $uniqueRHSA = @{}
        $uniqueData = @()
        foreach ($item in $rhsaData) {
...
Write-Host "   RHSA Vulnerability Tracker Script v3.2" -ForegroundColor Yellow
Write-Host "   With Original Source Line Column" -ForegroundColor Yellow
Write-Host "============================================" -ForegroundColor Yellow
Write-Host ""
...
# Extract RHSA IDs with their original source lines
Write-Host "Extracting RHSA IDs from input file..." -ForegroundColor Cyan
$rhsaDataList = Extract-RHSAIDsWithSource -FilePath $inputFilePath

if ($rhsaDataList.Count -eq 0) {
    Write-Host "No RHSA IDs found in the input file!" -ForegroundColor Red
    exit 1
}

Write-Host "Found $($rhsaDataList.Count) unique RHSA ID(s):" -ForegroundColor Green
Write-Host ""
foreach ($item in $rhsaDataList) {
    Write-Host "  RHSA ID: $($item.RHSA_ID)" -ForegroundColor Gray
    Write-Host "  Source : $($item.OriginalLine)" -ForegroundColor DarkGray
    Write-Host ""
}
...
foreach ($rhsaItem in $rhsaDataList) {
    $counter++
    Write-Host "[$counter/$total] $($rhsaItem.RHSA_ID)" -ForegroundColor White
    Write-Host "  Source: $($rhsaItem.OriginalLine)" -ForegroundColor DarkGray
...
Write-Host ""
Write-Host "Script completed! CSV Generated report can found with name 'rhsa_vulnerability_report*.csv' " -ForegroundColor Green
Write-Host ""
