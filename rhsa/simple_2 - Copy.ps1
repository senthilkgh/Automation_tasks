...
if($InputFile){if(Test-Path $InputFile){$fileContent=Get-Content $InputFile;$RHSAIds=@();foreach($line in $fileContent){if($line.Trim() -ne "" -and $line -notmatch "^#"){if($line -match '(RHSA-\d{4}:\d{4,5})'){$RHSAIds+=$matches[1]}}};$RHSAIds=$RHSAIds|Select-Object -Unique;Write-Host "Input: $InputFile" -ForegroundColor Yellow;Write-Host "Found: $($RHSAIds.Count) unique RHSA IDs" -ForegroundColor Yellow;Write-Host ""}else{Write-Host "ERROR: File not found" -ForegroundColor Red;exit 1}}elseif($RHSAIds){Write-Host "Processing: $($RHSAIds.Count) RHSA(s)" -ForegroundColor Yellow;Write-Host ""}else{Write-Host "ERROR: No RHSA IDs!" -ForegroundColor Red;Write-Host "";Write-Host "Usage:" -ForegroundColor Yellow;Write-Host "  .\rhsa-scraper.ps1 -RHSAIds 'RHSA-2026:2042'" -ForegroundColor Cyan;Write-Host "  .\rhsa-scraper.ps1 -InputFile 'rhsa-list.txt'" -ForegroundColor Cyan;Write-Host "";Write-Host "File format example:" -ForegroundColor Yellow;Write-Host "  Red Hat Update for brotli (RHSA-2026:2042)" -ForegroundColor Cyan;Write-Host "  Red Hat Update for curl (RHSA-2026:1350)" -ForegroundColor Cyan;Write-Host "";exit 1}
if($RHSAIds.Count -eq 0){Write-Host "ERROR: No RHSA IDs found in file!" -ForegroundColor Red;Write-Host "Check file format. Expected: Red Hat Update for package (RHSA-YYYY:NNNN)" -ForegroundColor Yellow;exit 1}
Write-Host "Extracted RHSA IDs:" -ForegroundColor Cyan;foreach($id in $RHSAIds){Write-Host "  - $id" -ForegroundColor White};Write-Host ""
