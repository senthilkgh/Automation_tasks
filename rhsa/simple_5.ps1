...
.NOTES
    ...
    Version: 3.0 - Using curl.exe with Fixed Date Parsing
#>
...
function Get-RHSADataFromErrata {
    ...
    Write-Host "    Patching Status: $($result.Patching_Status)" -ForegroundColor $(
        if ($result.Patching_Status -eq "vulnerability fixed on this month") { "Green" }
...
Write-Host "   RHSA Vulnerability Tracker Script v3.0" -ForegroundColor Yellow
Write-Host "   Using curl.exe with Fixed Date Parsing" -ForegroundColor Yellow
Write-Host "============================================" -ForegroundColor Yellow
Write-Host ""
...
Write-Host "  - Fixed Date < $($sixteenthOfLastMonth.ToString('yyyy-MM-dd'))  --> vulnerability fixed on this month" -ForegroundColor Green
Write-Host "  - Fixed Date >= $($sixteenthOfLastMonth.ToString('yyyy-MM-dd')) --> vulnerability will be fixed on next month patching." -ForegroundColor Yellow
Write-Host "  - No Fixed Date           --> not redhat product to fix" -ForegroundColor Red
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $results | Format-Table -Property @(
        @{Label="RHSA_ID"; Expression={$_.RHSA_ID}; Width=20},
        @{Label="Fixed_Date"; Expression={$_.Fixed_Date}; Width=12},
        @{Label="Severity"; Expression={$_.Severity}; Width=12},
        @{Label="Patching_Status"; Expression={$_.Patching_Status}; Width=50}
    ) -Wrap
